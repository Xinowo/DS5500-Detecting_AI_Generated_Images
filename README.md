...
> Run locally with `python demo/app.py` after you place pretrained checkpoints in `checkpoints/` (see [Workflow From Clone to Running](#workflow-from-clone-to-running)).
...
```bash
git clone https://github.com/Xinowo/DS5500-Detecting_AI_Generated_Images
cd DS5500-Detecting_AI_Generated_Images
...
1. To run the demo, download `checkpoints.zip` from **GitHub Releases** and extract it at the repo root.
2. To train on real data, download the Kaggle labeled images and extract them into `data/train_data/`.
3. `data/sampled_data_5k/` is optional. If you want lighter local runs, generate it locally from `data/splits/` plus `data/train_data/`, then override `--data_root data/sampled_data_5k`.
...
|----------|:---:|---|
| Config YAML files (`configs/`) | Yes | Included in this repo |
| Pre-computed split CSVs (`data/splits/`) | Yes | Included in this repo |
| Trained checkpoints (`checkpoints/`) | No | Download `checkpoints.zip` from the repo's **GitHub Releases** section, then extract it at the repo root |
| Training history CSV (`<outputs_dir>/metrics/`) | No | Generated during training under the `outputs_dir` set by each config |
| Full Kaggle image folder (`data/train_data/`) | No | Download from Kaggle and extract into `data/train_data/` |
| Optional 5k subset (`data/sampled_data_5k/`) | No | Generate locally from `data/splits/` if you want a smaller local dataset, then override `--data_root` |
| Python environment | No | `pip install -r requirements.txt` |

Here, `<outputs_dir>` means the run output folder set by a config, such as `outputs/resnet50/` or `outputs/vit_b16/`.
...
### Manual Downloads After Clone

The minimum manual assets you may need are:

1. `checkpoints.zip`
   Download from the repo's **GitHub Releases** section and extract it at the repo root.
2. `data/train_data/`
   Download the labeled images from Kaggle and extract them into `data/train_data/`.
3. `data/sampled_data_5k/`
   Optional. Create it yourself from `data/splits/` plus the Kaggle images if you want a smaller local dataset, then override `--data_root data/sampled_data_5k`.

Expected structure after manual setup:
...
```
data/sampled_data_5k/   # Optional 5k-image subset for lightweight local runs and demo examples
│   ├── train/          # 3,000 images
│   ├── validation/     # 1,000 images
│   └── test/           # 1,000 images
└── train_data/         # Full labeled Kaggle dataset (79,950 images) — too large for GitHub; download separately

checkpoints/            # Created automatically during training
outputs/                # Parent folder for run artifacts; each config writes into its own outputs_dir
```
...
## Workflow From Clone to Running

> **Environment note:**  
> - **Local**: the shipped training configs now default to `data/train_data` together with the committed split CSVs in `data/splits/`. If you also create `data/sampled_data_5k/`, you can override `--data_root data/sampled_data_5k` for lighter local runs.  
...
### 2. Download the manual assets you need

1. Download `checkpoints.zip` from the repo's **GitHub Releases** section and extract it at the repo root if you want to run the demo or use pretrained checkpoints.
2. Download the Kaggle labeled images and extract them into `data/train_data/` if you want to train on the full dataset.
3. Optionally create `data/sampled_data_5k/` from `data/splits/` plus `data/train_data/` if you want a smaller local dataset.

Notes:

- `data/splits/` is already committed in the repo.
- `data/train_data/` should contain image files directly, not `data/train_data/train_data/...`.
- `data/sampled_data_5k/` is optional and not required for the demo to work.
- the shipped configs now default to `data/train_data`
- only override `--data_root data/sampled_data_5k` if that optional subset exists locally

### 3. Smoke test (CPU, no GPU required)
...
### 4. Local training with the default full Kaggle image folder (`data/train_data`)

If you downloaded the full labeled dataset into `data/train_data/`, you can train directly with the shipped configs:
...
In this default mode, `training/train.py` loads `df_train.csv`, `df_val.csv`, and
`df_test.csv` from `data/splits/`, and `AIDataset` resolves each CSV entry such
as `train_data/abc.jpg` to `data/train_data/abc.jpg`.

Important: `data/train_data/` should contain the image files directly, not an
extra nested `train_data/` folder.

If you use the optional subset instead, the expected layout is:
...
### 5. Optional override: lighter local training with `data/sampled_data_5k`

If the optional 5k subset exists locally, override `--data_root`:
...
The current config output directories are:

- `configs/smoke_test.yaml` -> `outputs/smoke_test/`
- `configs/resnet50.yaml` -> `outputs/resnet50/`
- `configs/vit_b16.yaml` -> `outputs/vit_b16/`

### 6. Run the Gradio demo

The demo requires pretrained checkpoints. The easiest workflow is:

1. Download `checkpoints.zip` from **GitHub Releases**
2. Extract it at the repo root
3. Run:
...
The demo can auto-discover the newest checkpoint in each model directory, or you can pass explicit checkpoint paths with CLI flags:
...
If those files exist, the demo can run even if `data/sampled_data_5k/` does not exist.

### 7. HPC / Cluster training
...
### 8. Google Colab — notebooks or full-dataset training
...
2. Each model checkpoint directory must contain at least one file matching `best_model*.pth`:
   ...
   If you use the provided pretrained artifacts, download `checkpoints.zip` from **GitHub Releases** and extract it at the repo root.
...
Here, `<timestamp>` is the filename timestamp suffix in `YYYYMMDD_HHMMSS` format.
